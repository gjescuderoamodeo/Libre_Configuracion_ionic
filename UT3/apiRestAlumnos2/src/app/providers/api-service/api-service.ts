...
export class ApiServiceProvider {
    ...
    */
    ...
    buscarAlumnos(firstName:string, lastName:string): Promise<Alumno[]> {
        let promise = new Promise<Alumno[]>((resolve, reject) => {
            let url=this.URL + "/alumnos?first_name_like="+firstName+"&last_name_like="+lastName;
            console.log(url);
    ...
    getAlumnosPaginado(numPagina:number, numElementosPorPagina:number): Promise<Alumno[]> {
        let promise = new Promise<Alumno[]>((resolve, reject) => {
            let url=this.URL+ "/alumnos?_page="+numPagina+"&_limit="+numElementosPorPagina;
            this.http.get(url).toPromise()
                .then((data: any) => {
                    let alumnos = new Array<Alumno>();
                    data.forEach((alumno: Alumno) => {
